...
```text
C:\Users\gary1\OneDrive\桌面\obsidian\project開發\SignalForge\策略筆記\策略筆記撰寫框架.md
```

並依照該框架撰寫。策略筆記只保留策略相關內容：先解釋專業術語，再寫策略假設、進出場條件、主要參數、股價走勢解說圖、風險限制與下一步。不要把 `策略狀態`、`資料與回測`、`目前結果`、`變更紀錄` 放進策略筆記；這些內容應放在實驗紀錄、回測報告或專案進度筆記。
