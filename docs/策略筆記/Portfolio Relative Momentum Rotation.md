...
- Adjusted grid search、`top3 / breadth4 / maxconsec5 / liq500M` raw/adjusted comparison artifact 與 group regime validation 已補上。下一步不要繼續擴同一組 top-N / breadth / max-consecutive 小網格；優先做更高品質股票池、group-level breadth / regime validation，或直接設計能限制 realized group contribution concentration 的 gate。
