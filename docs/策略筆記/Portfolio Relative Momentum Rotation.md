...
- 目前主比較錨點分成四個：`top3 + breadth 42/min3` 是最高報酬錨點；`top4 + breadth 42/min3` 是風險調整折衷錨點；`top4 + breadth 42/min3 + max consecutive 5` 是績效 compare candidate；`top4 + breadth 42/min3 + max consecutive 5 + liquidity 500M/20 bars` 是最新 execution-aware compare candidate。`groupcap1/2`、更高 liquidity 門檻與 TWSE23 擴大股票池保留為 discard / compare-only / diagnostic 對照，不取代核心錨點。
