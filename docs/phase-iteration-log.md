...
| 2026-05-20 10:26 | Trace summary 加入 `first_reason` / `last_reason` | `*_trace_summary.json` 追加 deterministic `first_reason` / `last_reason`（schema_version=9），並在 `validate_signal_digest_csv(...)` 交叉驗證與 `*_signals.csv` 首尾 reason 一致（test-covered） | 110 -> 110；56 tests OK | keep |
