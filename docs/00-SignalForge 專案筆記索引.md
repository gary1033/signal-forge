...
- `portfolio rotation`：新增投組層級相對動能輪動工具 `tools\portfolio_rotation_sweep.py`，用 equal-weight buy-and-hold portfolio 作 benchmark，並輸出 IR、tracking error、active MDD 與自動 rolling windows；`monthly + 21 bars + top3` 是目前第一個 full-window 與 OOS 都有正 active return 的候選，但 24 個月 rolling 顯示 2021-2022 失敗，仍不能視為穩定營利。
