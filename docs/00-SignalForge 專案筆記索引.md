...
- `re-entry cooldown gate`：`portfolio_rotation_sweep.py` 已支援 `--reentry-cooldown-rebalances`，股票退出投組後需等待 N 次 rebalance 才能重新入選。TWSE35 adjusted `reentry6` 把 MDD 改到 `-28.76%`、active MDD 改到 `-25.74%`、min rolling excess 提高到 `18.63%`，但 full IR 降到 `1.046`、min rolling IR 精確值 `0.499582`，max rolling top3 group share 仍是 `100%`；promotion gate 仍是 `compare-only`，所以此 gate keep as tool，當前設定不升級。
- `balanced-quality universe selector`：`portfolio_rotation_universe_select.py` 已支援從 universe audit JSON 產生 deterministic 子股票池。TWSE35 `max_symbols_per_group=4` 選出 16 檔後，full IR `1.460`、3x IR `1.427`、active MDD `-21.80%`，但 min rolling IR `-0.367`、min rolling excess `-12.11%`；`max_symbols_per_group=3` 也失敗。因此 selector keep as tool，但把既有 TWSE35 越篩越小不是策略升級。
- `TWSE44 thin-group expansion`：用 TWSE 官方 OpenAPI 補 `shipping / steel / telecom / food / cement` 替代標的後，TWSE44 audit 有 `44` 檔、`33` 檔 eligible、`44` 檔 adjusted available；`shipping` 與 `steel` 替代性改善，single-member dominant window 降到 `0`。但 selected21 adjusted full IR 只有 `1.172`、MDD `-53.82%`、min rolling IR `-0.438`，promotion gate 仍是 `compare-only`。結論是資料流程 keep，TWSE44 selected21 / groupcap2 discard as current strategy upgrade。
- `group breadth filter`：`portfolio_rotation_sweep.py` 已支援 `--group-breadth-filter`，在每次 rebalance 只用當下可見的同群組成員 lookback return 檢查群組內正動能比例。TWSE35 adjusted 最佳掃描 `gb21/share0.50/min1` 把 full IR 提到 `1.867`、3x IR `1.848`、MDD 改到 `-32.85%`、min rolling IR `0.759`，但 max rolling top3 group share 仍是 `100%`，promotion gate 仍因 drawdown、group concentration、single-member dominant group 與 narrow group momentum 失敗；因此工具 keep，當前策略升級仍是 `compare-only` / discard。
- `group regime filter`：`portfolio_rotation_sweep.py` 已支援 `--group-regime-filter`，在每次 rebalance 只用當下可見的同群組成員等權 lookback return 檢查群組趨勢。TWSE35 adjusted 最佳掃描 `gb21/share0.50/min1 + greg21/r0.00` 把 full IR 提到 `2.005`、3x IR `1.987`、min rolling IR `1.034`、min rolling excess `48.53%`、active MDD `-19.44%`，但 MDD 仍是 `-32.85%`，max rolling top3 group share 仍約 `99.09%`，promotion gate 仍失敗；因此工具 keep，當前策略升級仍是 `compare-only` / discard。
