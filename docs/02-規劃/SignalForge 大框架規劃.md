...
- 目前 Phase 2 研究工具包含 `tools\multi_stock_target_state_sweep.py` 與 `tools\portfolio_rotation_sweep.py`。前者評估逐檔 target exposure，後者評估同一資金池的 portfolio-level 輪動；兩者都要檢查 1x / 3x 成本壓力與 walk-forward / OOS 分段。portfolio rotation 的風控 overlay 目前包含 market regime filter、breadth filter、volatility target、單檔連續入選上限、group cap、單成員群組 gate 與 liquidity gate，並已補逐股 attribution、group attribution、group exposure summary 與 concentration guard，必須用同一套 active-risk、symbol concentration、group contribution concentration 與 group exposure concentration gate 比較。
...
- 依 [[策略回測與優化評估準則|策略回測與優化評估準則]] 繼續補齊 benchmark-relative metrics；portfolio rotation 已補 IR / tracking error / active drawdown / rolling windows / market regime compare tool / breadth filter / volatility target compare tool / symbol attribution / group attribution / group exposure summary / concentration guard / 單檔連續入選上限 / group cap / 單成員群組 gate / TWSE23 擴大股票池診斷 / liquidity gate / dominant group exclusion 診斷 / canary universe 診斷 / adjusted price 診斷 / group regime validation / group breadth validation / promotion gate / universe audit，並已把 adjusted price 資料來源正式化為可重跑 per-symbol manifest、TWSE14 batch manifest 與 raw/adjusted comparison artifact。TWSE23 universe audit 顯示 23 檔中只有 16 檔通過歷史長度、流動性與群組成員數；若要求 adjusted CSV，現階段只有 12 檔 eligible。下一步重點轉向補齊 eligible 但缺 adjusted 的股票、較慢批次完成 TWSE30+、更高品質股票池、realized group contribution concentration gate，或更具體的 re-entry 條件。
