...
- 目前 Phase 2 研究工具包含 `tools\multi_stock_target_state_sweep.py` 與 `tools\portfolio_rotation_sweep.py`。前者評估逐檔 target exposure，後者評估同一資金池的 portfolio-level 輪動；兩者都要檢查 1x / 3x 成本壓力與 walk-forward / OOS 分段。portfolio rotation 的風控與排名 overlay 目前包含 market regime filter、breadth filter、group breadth filter、group regime filter、volatility target、ranking skip、ranking mode（`total-return` / `group-residual`）、單檔連續入選上限、group cap、單成員群組 gate、realized group contribution gate、re-entry cooldown gate 與 liquidity gate，並已補逐股 attribution、group attribution、group exposure summary、concentration guard、universe audit、universe selector 與 TWSE44 thin-group expansion，必須用同一套 active-risk、symbol concentration、group contribution concentration 與 group exposure concentration gate 比較。
...
- `TWSE44 thin-group expansion` 已正式測過：新增 `1103,1210,1227,1229,2023,2027,2609,2615,4904` 後，`shipping` 與 `steel` 的替代性改善，single-member dominant window 降到 `0`；但 selected21 adjusted full IR `1.172`、MDD `-53.82%`、min rolling IR `-0.438`，promotion gate 仍是 `compare-only`，因此資料流程 keep、策略升級 discard。
...
- TWSE44 thin-group expansion 證明補替代標的能修 single-member dominant 診斷，但不能自動改善策略品質；後續不要只補低流動性 food / cement，也不要只加 group cap。下一步應轉向可驗證的 drawdown / risk-off、shipping regime 風險、market / industry regime filter，或搜尋新的策略 family。
