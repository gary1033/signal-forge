...
- 依 [[策略回測與優化評估準則|策略回測與優化評估準則]] 繼續補齊 benchmark-relative metrics；portfolio rotation 已補 IR / tracking error / active drawdown / rolling windows / market regime compare tool / breadth filter / volatility target compare tool，下一步重點轉向更大股票池、1x / 2x / 3x cost stress 固定報表，或更具體的 re-entry / canary universe 條件。
...
- 針對 portfolio rotation，下一步不要直接宣稱穩定營利；14 檔 `breadth 42/min3` 雖讓 full-window IR 約 `1.417` 並讓 1x/2x/3x 成本與 6 個 rolling windows 都保持正 excess，但仍可能集中在少數高波動股票，且 TWSE STOCK_DAY 資料未還原權息。後續應先補 per-symbol / per-window selection attribution，再處理 adjusted price 或更大股票池。
