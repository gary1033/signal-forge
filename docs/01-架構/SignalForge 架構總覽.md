...
| Portfolio rotation sweep | `tools\portfolio_rotation_sweep.py` | Phase 2 portfolio-level 評估工具，將多檔股票視為同一資金池，檢查相對動能輪動是否勝過 equal-weight buy-and-hold portfolio，並輸出 Information Ratio、tracking error、active max drawdown、自動 rolling windows 與可選 market regime filter。 |
