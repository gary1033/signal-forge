...
- `tools\build_twse_adjusted_ohlcv.py` 已正式化 adjusted price 資料來源：用 Yahoo chart `adjclose / close` 比例調整 TWSE source OHLC，保留 TWSE source volume，並輸出 deterministic manifest 供後續 raw / adjusted-ratio 回測對照。
