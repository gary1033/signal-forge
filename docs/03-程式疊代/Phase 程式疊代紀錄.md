...
- Target-state 主線先以 `absolute-momentum` 作 compare-only 錨點；逐檔 target-state 已證明 benchmark-relative 問題仍存在。Portfolio-level rotation 是目前較有希望的新主線；14 檔 `breadth 42/min3 top4 max consecutive 5 liquidity 500M/20 bars` 是最新 execution-aware compare candidate，但 concentration guard 顯示部分 rolling window 仍有單檔集中風險。sector/group cap 已測但未解；TWSE23 擴大股票池降低 concentration 但犧牲 edge。下一步應測 adjusted price、較慢批次完成 TWSE30+、canary universe 或 group regime / group exposure diagnostic，並用 Information Ratio / active drawdown / symbol concentration / group concentration gate 防止單一 OOS window 過度樂觀。
